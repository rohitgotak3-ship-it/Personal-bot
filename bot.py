...
class MainDashboard(discord.ui.View):
    ...
    async def guard(self,i):
        if not is_master(i): await i.response.send_message(f'{ERROR} **𝐃𝐀𝐒𝐇𝐁𝐎𝐀𝐑𝐃 𝐀𝐂𝐂𝐄𝐒𝐒 𝐃𝐄𝐍𝐈𝐄𝐃**',ephemeral=True); return False
        return True
...
async def dashboard(i):
    if not is_master(i): return await i.response.send_message(f'{ERROR} **𝐃𝐀𝐒𝐇𝐁𝐎𝐀𝐑𝐃 𝐀𝐂𝐂𝐄𝐒𝐒 𝐃𝐄𝐍𝐈𝐄𝐃**',ephemeral=True)
    await i.response.send_message(f'{CROWN} **𝐋𝐈𝐆𝐇𝐓𝐍𝐄𝐒𝐒 𝐃𝐀𝐒𝐇𝐁𝐎𝐀𝐑𝐃**\n{ARROW} Select a module below.',view=MainDashboard(),ephemeral=True)

@bot.tree.command(name='announcement',description='Open announcement setup.')
async def announcement(i):
    if not is_master(i): return await i.response.send_message(f'{ERROR} Master access only.',ephemeral=True)
    await i.response.send_message(f'{CROWN} **𝐀𝐍𝐍𝐎𝐔𝐍𝐂𝐄𝐌𝐄𝐍𝐓 𝐒𝐄𝐓𝐔𝐏**',view=AnnouncementView(),ephemeral=True)
